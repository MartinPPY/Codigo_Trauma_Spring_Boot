...
public interface UserRepository extends CrudRepository<User, Long> {
    ...
    @Query("SELECT new com.martin.codigo.trauma.app.models.UserDto(u.name,u.lastname,u.phone,u.email,u.availability) FROM User u WHERE u.role.id = 2 ORDER BY u.name,u.lastname")
    List<UserDto> findAllMedics();

    // Query para traer solo medicos y filtrar por disponibilidad
    @Query("SELECT new com.martin.codigo.trauma.app.models.UserDto(u.name,u.lastname,u.phone,u.email,u.availability) FROM User u WHERE u.role.id = 2 AND u.availability = ?1 ORDER BY u.name,u.lastname")
